@echo off
rem Fetches the latest pushed claude/fixing commit and serves it from the
...
setlocal
cd /d "%~dp0" || exit /b 1
git fetch origin claude/fixing || exit /b 1
set "OTB_STABLE_SHA="
...
echo.
echo Serving claude/fixing commit %OTB_STABLE_SHA% from ..\otb-chess-stable
echo.
