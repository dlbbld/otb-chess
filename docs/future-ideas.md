...
## A permissive "crazy chessboard" mode

OTB Chess models the board's *technical posture*: it observes silently and doesn't enforce rules during play. As scope grows toward modelling more of over-the-board chess's untidy social reality (distraction, complaint, arbiter judgement, freak cases), one speculative direction is a **"crazy chessboard"** mode — a board that lets players do everything, including the weird and unwise things that physical chess silently permits and FIDE only loosely constrains.

Such a mode would mark a shift in framing: from "minimal enforcement" toward "celebrated freedom + structured consequence." Not decided, just on the table.
