 ...
 * @param opponentMessage      message broadcast to the opponent ({@code null} when the opponent
 *                             should not be notified — e.g. invalid-SAN re-prompt)
...
public record DrawClaimResult(
    boolean accepted,
    String message,
    Optional<String> opponentMessage,
    Optional<String> gameEndDescription,
  ...
  public static DrawClaimResult invalidMove(String message) {
    return new DrawClaimResult(false, message, Optional.empty(), Optional.empty(),
        Optional.empty(), true, false);
  }

  /**
   * Used for non-FIDE-claim error paths (e.g. game not in progress, not-on-move, second claim
   * on same move): rejection without converting to a draw offer.
   */
